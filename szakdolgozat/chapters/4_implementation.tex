    ...
    for group in range(len(groups)):
        if groups[group].name == simple_groups[simple_group].name:
            ...
            if ok or counter == 10000:
                groups[group].room =
                 simple_groups[simple_group].room.name
\end{python}
...
def targetFunction(entity):
    ...
    for period in range(NUM_GENS):
        ...
        if entity['gens']['periods'][period] == time_windows['f'][0]:
            ...
            diff += (SCALE_MAX - extremisms + 1) *
             len(periods[period]['assignments'])
...
(\textit{MUTATION\_RATE}) értéke határozza meg a különböző esetek bekövetkezésének valószínűségét, melyek mind külön metóduson keresztül kerülnek megvalósításra. Egy példaként létrehozott gyerek elemet \aref{tab:gyerek}. táblázatban láthatunk.
...
Az alkalmazás rétegeit, az egyes műveletekhez tartozó feldolgozási folyamatot egy szekvenciadiagram formájában \aref{fig:sequence}. ábrán láthatjuk.
...
\item a több-több kapcsolatok megfelelően tárolódtak-e le, különös tekintettel az osz\-tály-tantárgy kapcsolatokra, ahol a kapcsolótáblához kötődő attribútumok is vannak
...
\item csak az adott órarenden belüli tantárgyak, tantermek, tanárok és osztályok kerül\-nek-e felhozásra, illetve csak az adott felhasználó órarendjei vannak-e meg
